...
int main(int argc, char *argv[]) {
    ...
    char* mode; // "read\0" 또는 "write\0"를 담기 위한 배열
    ...
    if (strcmp(param3, "nru")==0){ // NRU
        NRU q;
        initNRU(&q, param2); // page table 초기화
        // 파일 읽기
        while (fscanf(file, "%d %s", &pid, mode) == 2) {
            TOTAL_ACCESS += 1;
            if (strcmp(mode, "read") == 0) // read인 경우
            ...
            // page table에 pid가 있는지 확인
            // printf("%d \n",pid);
            if (NRU_checkTable(&q, pid, mode)){ // 문제가 있다.
                // 테이블에 특정 pid가 있다면 HIT
                // printf("HIT %d\n",pid);
                HIT += 1;
                // continue;
                ...
                // FAULT 일 경우 -> FIFO algorithm
                // printf("FAULT %d\n", pid);
                NRU_Append(&q,pid, mode);
                FAULT += 1;
            }
            // printDoublyLinkedList(&q);
        }
    }
