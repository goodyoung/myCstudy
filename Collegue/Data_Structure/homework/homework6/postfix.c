...
void postfixPush(char x); // 기존 postfixpush -> postfixPush로 변경, postfix 스택에 data를 푸시하는 함수를 선언
...
void toPostfix()
{
	...
	while(*exp != '\0')
	{
		...
		if (token==rparen){ // token이 ')' 이면
			...
			if (token != lparen){ // lparen일 때를 한 이유는 in-comming일 때는 lparen의 우선순위가 제일 높기 때문에 무조건 push가 되어야 한다.
...
void evaluation()
{
	...
	while (*exp){
		...
		if (token == operand){ // token이 oprend이면
			...
			// evalStack에 2번의 pop을 통해 2개의 값 각각 op2,op1에 저장
			op2 = evalPop();
			op1 = evalPop();
			switch(token){
				case plus:
